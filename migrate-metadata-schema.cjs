...
async function main() {
    ...
    try {
        ...
        for (const category of categories) {
            try {
                const oldMetadata = category.metadata;
                // Check if migration is needed
                const needsMigration = oldMetadata.followers_range && Array.isArray(oldMetadata.followers_range);
                if (!needsMigration) {
                    console.log(`⏭️  Skipping ${category.name} (${category.categoryType}) - already migrated`);
                    skippedCount++;
                    continue;
                }
                ...
                console.log(`   Old: followers_range: [${oldMetadata.followers_range[0]}, ${oldMetadata.followers_range[1]}], price: ${oldMetadata.price}`);
                console.log(`   New: follower_range: {min: ${newMetadata.follower_range.min}, max: ${newMetadata.follower_range.max}, display: "${newMetadata.follower_range.display}"}, base_price: ${newMetadata.pricing.base_price}\n`);
                migratedCount++;
            }
            catch (error) {
