...
const CACHE_KEY = 'homepage:orders-fulfilled-all-time';
...
export const GET: RequestHandler = async ({ setHeaders }) => {
	...
	if (cached !== null) {
		return json({ success: true, data: { ordersFulfilled: cached } });
	}

	const ordersFulfilled = await prisma.order.count({
		where: {
			paymentStatus: { in: [...CONFIRMED_PAYMENT_STATUSES] }
		}
	});

	serverCache.set(CACHE_KEY, ordersFulfilled);

	return json({ success: true, data: { ordersFulfilled } });
};
