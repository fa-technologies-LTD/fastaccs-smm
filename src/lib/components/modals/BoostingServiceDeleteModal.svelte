<script lang="ts">
	...
	interface Props {
		open: boolean;
		service: CategoryMetadata | null;
		loading?: boolean;
		onClose: () => void;
		onConfirm: () => void;
	}

	let { open, service, loading = false, onClose, onConfirm }: Props = $props();
</script>
					...
					<div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
						...
						<div class="mt-2">
							<p class="text-sm" style="color: var(--text-muted);">
								Are you sure you want to delete <strong>{service?.name}</strong>? This action cannot
								be undone.
							</p>
							<p class="mt-2 text-sm text-red-600">
								<strong>Warning:</strong> Existing orders for this service keep their data, but
