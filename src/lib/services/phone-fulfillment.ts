...
export interface PhonePollResult {
	status: 'awaiting_sms' | 'received' | 'refunded' | 'expired' | 'unknown';
	phoneNumber?: string;
	otp?: string;
	message?: string;
...
/**
 * Poll hub-man for the OTP on an awaiting rental; persist it when it arrives.
 * If the activation window has passed with no SMS, cancel + refund.
 */
export async function pollPhoneRentalSms(orderItemId: string): Promise<PhonePollResult> {
	...
	if (rental.status !== 'awaiting_sms' || !rental.hubOrderUuid)
		return { status: 'awaiting_sms', phoneNumber: rental.phoneNumber ?? undefined };
	...
	if (sms && sms.otp) {
		...
		if (claim.count > 0) {
			await prisma.orderItem
				.findUnique({ where: { id: orderItemId }, select: { orderId: true } })
				.then((oi) => {
					if (oi)
	...
	if (Date.now() > deadline) {
		await cancelAndRefundRental(orderItemId, 'No code arrived in time — refunded to store credit');
...
/** Cancel the rent on hub-man (refunds our balance) then refund the customer. Idempotent. */
export async function cancelAndRefundRental(orderItemId: string, description: string): Promise<void> {
	const rental = await prisma.phoneRental.findUnique({ where: { orderItemId } });
	if (!rental || TERMINAL_STATUSES.has(rental.status)) return;

	if (rental.hubOrderUuid) {
		await hubman.cancelRent(rental.hubOrderUuid).catch((error) => {
	...
	if (oi) await refundPhoneOrderToStoreCredit(oi.orderId, description, 'timeout');
}
...
/** Sweep awaiting rentals whose window has closed (called by cron). Returns count acted on. */
export async function sweepExpiredPhoneRentals(): Promise<number> {
	...
	void pricing; // pricing consumed inside pollPhoneRentalSms per-rental
	return acted;
}
